...
**创业模拟器** — Godot 俯视角办公室建造与创业经营模拟游戏。

你是 SaaS 创始人，种子轮 100 万。未来 Godot 版的主线是经营公司取得收益、完成目标和解锁成就；办公室区域、设施升级、员工成长和员工日常模拟是支撑公司发展的系统。
...
> 近期 Godot 主线暂不做 AI 玩法、自由指令、AI 角色对话或模型接入。
> Godot 核心交互将支持暂停、正常速度、二倍速、三倍速；员工会有能力、特长、随机正负特性、疲劳、情绪、健康和日常需求。
...
当前 CLI 原型仍支持自然语言输入决策，每回合 ≤5 个动作。它只作为规则参考和回归测试入口，不代表未来 Godot 核心玩法：
...
```bash
pytest tests/ -v    # 463 passed
```

测试覆盖：
- **平衡测试**：5种策略 × 12回合自动运行，保留结局多样性回归，并验证至少一条可通往 A 轮成功的参考路径
...
**真实内测反馈修复版** — 当前 CLI/飞书体验验证层，不代表未来 Godot 核心玩法。
...
*体验验证版 Alpha 1.9.1 — 463 tests passed；后续开发以 Godot 表现层和 C# Core 规则迁移为主。*
