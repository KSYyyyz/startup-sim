...
后续项目推进采用“前端/美术 Agent + 后端/规则 Agent + 共同合同层”的协作方式。

### 角色分工

1. 前端/美术 Agent
   - 负责 `frontend/`、`design-assets/`、前端体验文档。
   - 负责办公室主场景、行动卡、布局降噪、image-2 资源库、角色头像/房间/事件气泡、Vercel 线上体验。
   - 不允许自行发明现金、用户、产品分、估值、股权等结算逻辑。
   - 不允许把新玩法规则写死在 React 组件里；新增玩法内容必须优先进入玩法数据或合同层。
...
1. 后端/规则 Agent 优先定义或扩展合同，前端/美术 Agent 根据合同展示。
2. 前端不得修改合同字段语义；如需新增字段，先写入合同文档并补测试。
3. 后端不得把前端展示结构写进规则层；规则层输出事实，展示层决定表现。
