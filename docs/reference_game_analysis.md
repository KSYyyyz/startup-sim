# 参考游戏分析

状态：参考基线
日期：2026-05-18
范围：本地只读目录扫描

## 1. 边界说明

本文记录 Startup Sim 可以从本地安装的参考游戏中学习什么，同时明确不复制任何专有内容。

允许做的事：

- 研究文件夹组织、公开的 Mod/本地化结构、存档/配置/日志约定，以及高层交互模式。
- 将观察结果用于设计 Startup Sim 自己的原创系统、UI 流程、文档、资产管线和桌面分发路线。
- 在安装目录正常可见的范围内，引用文件和系统结构的存在。

不允许做的事：

- 不把参考游戏的美术、音频、文本、代码、数据库或游戏专有名称复制进 Startup Sim。
- 不反编译、不解包、不绕过 DRM，也不逆向闭源实现。
- 不一比一复刻专有机制。所有观察都必须转化为 Startup Sim 自己的原创设计原则。

## 2. 本地参考扫描

### 2.1 Mad Games Tycoon 2

本地路径：`D:\XDWork\Game\Mad.Games.Tycoon.2.Build.20855306`

观察到的结构：

- Unity 运行时布局：`Mad Games Tycoon 2_Data`、`MonoBleedingEdge`、`UnityPlayer.dll`。
- `Mad Games Tycoon 2_Data\Extern` 下存在外部资产文件夹，包括公司 Logo、功能图标、品类图标、平台图标、截图和文本。
- 数据目录中包含大型 Unity 资产包、场景文件和资源文件。

对 Startup Sim 的价值：

- 很适合作为“办公室/公司经营游戏”的形态参考：工作空间本身就是主要玩法界面。
- 强化了 Startup Sim 的方向：房间、设施、员工、项目和市场反馈应该可见、可点击、可逐步成长。
- 可作为公司从小团队成长为更大运营机器的产品形态参考。

安全借鉴结论：

Startup Sim 应该构建自己的办公室层，让每个房间都代表一种商业能力：产品、销售、团队、董事会、基础设施、市场，以及后续的财务/合规。可以参考的是交互模式，不是资产或具体规则。

### 2.2 STONKS-9800

本地路径：`D:\XDWork\Game\STONKS9800`

观察到的结构：

- GameMaker 风格的包结构，包含 `data.win`、可执行文件、选项/配置文件、背景、音乐、调色板、姓名、本地化文件和 Mod 文件夹。
- 存在公开的 Mod 表面：`modding_guide.txt`、`mods\test_mod.meow`、`make your own localization` 和 `mod_uploader`。
- 类数据文件夹与编译后的游戏核心内容有清晰分离。

对 Startup Sim 的价值：

- 是本地参考中最适合学习 Mod/本地化/数据扩展性的案例。
- 展示了“玩家可见、玩家友好”的自定义表面价值：Mod、本地化、姓名、调色板和上传工具。
- 提供了一个有用模式：把玩家可编辑内容放在编译核心之外。

安全借鉴结论：

Startup Sim 应该逐步把原创游戏数据与运行时代码分离：

- `godot/StartupSimGodot` 存放运行时玩法 UI。
- `design-assets/` 存放生成资产的 prompt、导出和登记信息，Godot 使用稳定导出。
- `data/gameplay` 或等价目录存放原创房间、行动、事件、投资人、竞品和角色定义。
- `data/locales` 在文本表面稳定后承载本地化内容。
- 后续可以提供文档化的 Mod 或剧本格式，用于自定义创业行业、投资人、竞品和事件。

### 2.3 历史模拟器：崇祯

当前阶段先忽略该参考。Startup Sim 近期不做自由指令、AI 角色对话、AI 事件叙事或模型接入，因此不把它纳入当前玩法分析基线。

### 2.4 Game Dev Story / 游戏开发物语

本地路径：`D:\XDWork\Game\Game.Dev.Story.Build.8596453`

观察到的结构：

- Unity/IL2CPP 运行时布局，包含 `KairoGames_Data`、`GameAssembly.dll`、`UnityPlayer.dll` 和较紧凑的资源文件。
- 安装形态相对小而自洽。
- 顶层扫描中没有看到公开 Mod 或本地化表面。

对 Startup Sim 的价值：

- 很适合作为紧凑经营循环参考：小屏幕、快决策、清晰进度反馈和长期重玩价值。
- 提醒我们：商业模拟不需要一开始暴露所有系统，也可以让玩家感到有策略性。
- 支持 Startup Sim “复杂机制渐进解锁”的原则。

安全借鉴结论：

Startup Sim 的早期游戏应该保持易读、轻快。即使办公室视图越来越丰富，每个月仍应通过少数几个有意义的 CEO 决策来推进，而不是变成密集的管理表格。

### 2.5 Game Dev Tycoon

本地路径：`D:\XDWork\Game\Game Dev Tycoon`

观察到的结构：

- NW.js / Web 技术桌面分发结构，包含 `nw.exe`、`package.json`、`defaultBrowser.html`、`js`、`css`、`images`、`audio`、`i18n` 和 `mods`。
- 窗口配置以桌面端为目标，最小尺寸为 1024 x 768。
- 可见 `mods\gdt-modAPI` 包，包含包元数据、公开 API 文件夹、示例，以及指向官方 Mod API 文档的 README。
- 安装目录中包含本地化和许可证文件夹，清楚展示了扩展和分发边界。

对 Startup Sim 的价值：

- 很好地证明了模拟经营游戏可以用 Web 技术作为桌面应用发布。
- 对“如何通过可见包边界保持可 Mod 或可扩展”很有参考价值。
- 对桌面优先 UI 尺寸、本地化目录、Mod API 分离和 Web 运行时打包都有参考意义。

安全借鉴结论：

Startup Sim 当前已经切换到 Godot 主线。更有价值的启发是：把办公室、房间、行动、角色、事件和剧本定义做成清晰的数据边界，让 Godot 表现层可以消费稳定合同，同时不复制底层规则。

## 3. 对产品方向的影响

### 3.0 公开玩法资料补充

在本地目录扫描之外，补充查看了公开商店页和测评资料。结论如下：

- 《Software Inc.》最接近 Startup Sim 的目标形态。它把员工技能、员工需求、团队、办公室、服务器和产品发布放在同一个公司经营框架里。可学习的不是具体数值，而是“员工能力和办公室条件共同决定产品交付”的大方向。资料来源：[Steam 商店页](https://store.steampowered.com/app/362620)。
- 《Startup Company》强调 CEO 经营公司、组织办公室、保持员工满意和可 Mod 内容。可学习点是办公室和员工系统要服务公司成长，同时避免陷入“产出抽象资源点”的点击器循环。资料来源：[Steam 商店页](https://store.steampowered.com/app/606800/startup_company/)。
- 《Mad Games Tycoon 2》强调从小工作室扩张到多部门公司，办公室空间和员工数量随公司阶段成长。可学习点是每次扩张都应该带来清晰的新能力和新风险。资料来源：[Steam 商店页](https://store.steampowered.com/app/1342330/Mad_Games_Tycoon_2/)。
- 《Two Point Hospital》展示了员工需求、房间布局、设施品质和服务效率之间的关系。可学习点是员工需求应该形成可读瓶颈，而不是让玩家陷入无意义的照料。资料来源：[PC Gamer 测评](https://www.pcgamer.com/two-point-hospital-review/)、[GameSpot 测评](https://www.gamespot.com/reviews/two-point-hospital-review-laughter-is-the-best-med/1900-6416981/)。
- 《Little Big Workshop》展示了工位、机器、房间、员工、生产订单和休息需求如何形成可视化运营循环。可学习点是员工成长和设施升级必须能被玩家从空间流动中看见。资料来源：[Video Chums 测评](https://videochums.com/review/little-big-workshop)、[The Lost Noob 测评](https://www.lostnoob.com/reviews/little-big-workshop/)。

这些资料进一步确认：Startup Sim 的主线应该是公司经营结果，员工成长、员工需求、设施升级和办公室布局都是经营杠杆。员工成长系统必须让玩家看到“人变强了，公司也真的变强了”，而不是只多出一组养成数值。

### 3.1 办公室层就是主游戏棋盘

当前 Godot 办公室层应从一开始就避免“仪表盘构图”，直接向“可玩的经营空间”推进。

下一步方向：

- 房间不是装饰区域，每个房间都是一个系统入口。
- 房间状态应该表达经营压力：忙碌、阻塞、改善、风险或等待决策。
- 员工状态和房间事件应该成为商业状态变化的可见反馈。
- 月度变化既要显示在信息面板里，也要反映在办公室场景中。

### 3.1.1 员工成长必须服务经营主线

参考游戏显示，员工成长最容易出现两个问题：一是变成无脑堆等级，二是变成疲劳、厕所、情绪等微管理。Startup Sim 应该走第三条路：

- 员工通过岗位经验、培训、项目突破和导师带教成长。
- 成长结果首先改变公司经营能力，例如研发速度、销售转化、客户满意、服务器稳定和组织效率。
- 员工详情页展示个人成长，月报只展示和公司结果有关的成长。
- 培训必须有短期机会成本，避免玩家无脑点击升级。
- 每个员工有潜力和适配方向，让“培养谁、放在哪个岗位”成为有趣选择。
- 负面特性只能缓和，不能被免费清除，保留角色记忆点和团队管理取舍。

验收上，第一版不追求复杂职业树，只要求有明确成长字段、可见成长事件、岗位适配提示和至少一种培训动作。

### 3.2 数据驱动内容应该尽早开始

STONKS-9800 的公开 Mod/本地化结构提供了最有用的工程启发。Startup Sim 不应该永远把所有房间、行动、投资人、竞品、事件和角色硬编码在代码里。

推荐方向：

- 将房间和行动定义逐步迁移为结构化数据。
- 确定性规则仍然留在核心引擎中。
- Godot 读取稳定的数据定义，用于展示标签、说明、取舍标签和房间元数据。
- 在不过早承诺公开 Mod 的前提下，为原创剧本包预留结构。

### 3.3 桌面分发路线应该保持开放

这些参考游戏展示了几种可行的打包形态：

- 偏大型商业引擎的重桌面游戏。
- GameMaker 风格的可执行文件，并暴露数据/Mod 文件夹。
- 使用 Web 技术的 Electron 桌面游戏。
- 使用 Web 技术的 NW.js 桌面游戏。
- 使用 Godot 的轻量独立游戏。

Startup Sim 后续不再切换到 Unity 或继续把 Web 作为最终外壳。当前项目受益于可复用的 Python 规则参考、已经成型的 C# Core，以及 Godot 4.6.x .NET 的桌面开发路径。正确路径是：

1. 先让 Godot 本地版本完成办公室到回合结算的可玩闭环。
2. 保持 C# Core 与 Godot 表现层边界清晰。
3. 增加本地存档和导出路径。
4. 不再恢复 Web/Vercel 前端主线。

## 4. 路线调整建议

### 4.1 近期

- 继续保持 Godot 桌面端优先。
- 把办公室场景做成互动棋盘，而不是静态插图。
- 增加可见的房间状态和活动脉冲。
- 让董事会、竞品和办公室反馈更像游戏事件。
- 保持单回合决策足够紧凑，保留《游戏开发物语》式的轻快节奏。
- 建议默认折叠。
- 玩家可见文案继续使用“现金流可支撑时间”。

### 4.2 下一步游戏数据层

引入一个小型原创数据层，用于 Godot 玩法定义：

- 房间。
- 房间行动。
- 行动取舍标签。
- 董事会压力回应模板。
- 竞品压力回应模板。
- 办公室反馈信号类型。
- 剧本元数据，让未来内容包可以描述行业、初始条件和竞品组合，而不需要重写 UI 代码。

这层应该是数据驱动的，但仍然通过 C# Core / TurnEngine 执行。Godot 可以提出指令，确定性引擎仍然负责状态变化。

### 4.3 后续桌面游戏阶段

在不提前绑定最终打包方案的前提下，为可分发独立游戏做准备：

- 保持存档数据可序列化。
- 尽量避免只能在浏览器中成立的假设。
- 文档化必要的环境变量和离线 fallback 行为。
- 将 image-2 生成资产继续放在项目设计资产库中。
- 在承诺公开 Mod 之前，先让本地化目录和内容包目录可见、可文档化。
- Steamworks、成就、云存档和 Mod 上传器都推迟到可玩的 Alpha 循环之后。

## 5. 不要做什么

- 不复制参考游戏的图片、音频、文本、Logo、角色名或具体事件脚本。
- 不解包或检查闭源应用包的实现。
- 不再推进 Unity 主线。
- 不再把 Web/PixiJS 作为最终表现层。
- 在核心游戏还不好玩前，不提前过度建设 Mod 系统。
- 不引入 AI 叙事或自由指令作为近期玩法。

## 6. Startup Sim 设计规则

参考游戏再次确认了当前原则：

> 真实商业是模拟地基，游戏感是玩家面对的体验。

对 Startup Sim 来说，这意味着：

- 办公室就是棋盘。
- 房间就是控制入口。
- 数值结果由确定性系统决定。
- 数据定义应具备足够复用性，以便支持剧本、本地化和未来桌面分发。
